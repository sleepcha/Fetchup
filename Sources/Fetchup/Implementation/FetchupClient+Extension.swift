...
public extension FetchupClient {
    ...
    func fetchDataTask<T: APIResource>(
        ...
        completion: @escaping (Result<T.Response, FetchError>) -> Void
    ) -> URLSessionDataTask {
        ...
        task.delegate = FetchupClientTaskDelegate(configuration, cacheMode: cacheMode) {
            let result = processResponse(data: $0, response: $1, error: $2).flatMap { data in
                resource.decode(data).mapError(FetchError.decodingError)
            }
            completion(result)
        }
    ...
    func cached<T: APIResource>(_ resource: T, isValid: (Date) -> Bool) -> Result<T.Response, CacheError> {
        ...
        return resource.decode(response.data).mapError(CacheError.decodingError)
    }
